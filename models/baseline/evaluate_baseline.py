...
MODEL_LOAD_PATH = "baseline_weights.pth"
...
def evaluate(model, dataloader, vocab):
    ...
    with torch.no_grad():
        for input_seq, input_lengths, target_seq, _ in tqdm(dataloader, desc="Evaluating"):
            ...
            # Predict returns a list of lists of token indices
            predicted_indices_list = model.predict(input_seq, input_lengths, vocab, max_length=target_for_comparison.shape[1])
